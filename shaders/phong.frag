...
vec3 Shade(vec3 lightIntensity, vec3 lightDir, vec3 normal, vec3 viewDir, vec3 diffuseColor, vec3 specularColor, float shininess) {
    // your code here:
    // vec3 ambient = lightIntensity * vec3(texture(u_HeightMap, v_TexCoord));
    float diff = max(dot(normal, normalize(lightDir)), 0.0);
    vec3 diffuse = diffuseColor * diff * vec3(texture(u_DiffuseMap, v_TexCoord));
    vec3 reflectDir = -reflect(lightDir, normal);
    float spec = pow(max(dot(normalize(viewDir), reflectDir), 0.0), shininess);
    vec3 specular = specularColor * spec * vec3(texture(u_SpecularMap, v_TexCoord));
    if (u_UseBlinn){
        vec3 halfwayDir = normalize(lightDir + viewDir);
        spec = pow(max(dot(normal, halfwayDir), 0.0), shininess);
        specular = specularColor * spec * vec3(texture(u_SpecularMap, v_TexCoord));
    }
    return vec3(lightIntensity * (diffuse + specular));
}
