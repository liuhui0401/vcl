...
namespace VCX::Labs::Rendering {
    ...
    bool IntersectTriangle(Intersection & output, Ray const & ray, glm::vec3 const & p1, glm::vec3 const & p2, glm::vec3 const & p3) {
        ...
        glm::mat3 M = glm::mat3(-ray.Direction, e1, e2);
        float det = glm::determinant(M);
        if (det < 0.0001f)
            return false;
        glm::mat3 M1 = glm::mat3(T, e1, e2);
        glm::mat3 M2 = glm::mat3(-ray.Direction, T, e2);
        glm::mat3 M3 = glm::mat3(-ray.Direction, e1, T);
        output.t = glm::determinant(M1) / det;
        output.u = glm::determinant(M2) / det;
        output.v = glm::determinant(M3) / det;
        if (output.u < 0.0f || output.v < 0.0f || output.u + output.v > 1)
            return false;
        return true;
    }
            ...
            for (const Engine::Light & light : intersector.InternalScene->Lights) {
                ...
                if (light.Type == Engine::LightType::Point) {
                    ...
                    if (enableShadow) {
                        ...
                        if (hit.IntersectState && hit.IntersectAlbedo.w >= 0.2 && glm::length(hit.IntersectPosition - pos) < glm::length(l))
                            flag = false;
                    }
                } 
                ...
                // your code here
                if (flag){
                    float diff = std::max(glm::dot(n, glm::normalize(l)), 0.0f);
                    result += light.Intensity * kd * attenuation * diff;
                    glm::vec3 reflectDir = glm::normalize(l + ray.Origin - pos);
                    float spec = pow(std::max(glm::dot(n, reflectDir), 0.0f), shininess);
                    result += light.Intensity * ks * attenuation * spec;
                }
            }
